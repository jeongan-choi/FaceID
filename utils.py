...
def crop_face_from_id(cv_image, weight_path="weights"):
    ...
    predictor = dlib.shape_predictor(os.path.join(weight_path, 'shape_predictor_68_face_landmarks.dat')) #수정
...
#     tess.pytesseract.tesseract_cmd = r"/opt/homebrew/Cellar/tesseract/5.4.1/bin" #수정
...
def get_idnum(cv_image):
    """Extract birth year from ID card using Tesseract"""
    tess.pytesseract.tesseract_cmd = r'/opt/homebrew/bin/tesseract'  # Update the path accordingly
    config = ("-l kor+eng --oem 3 --psm 4 -c preserve_interword_spaces=1")
    text = tess.image_to_string(cv_image, config=config)
    try:
        text = re.compile(r"\d{6}").search(text).group()
